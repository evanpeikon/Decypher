...
def test_mediation(data, cause, effect, mediator, max_lag=2, p_threshold=0.05):
    ...
    try:
        ...
        # Consider mediated if direct effect is not significant and mediation ratio is substantial
        is_mediated = (direct_effect_p > p_threshold) and (mediation_ratio > 0.5)
...
def prune_mediated_edges(G, data, mediation_threshold=0.6, max_lag=2):
    """
    Remove edges that are primarily mediated by other variables.
    
    Parameters:
    - G: NetworkX DiGraph
    - data: Original DataFrame
    - mediation_threshold: Minimum mediation ratio to consider edge as mediated
    - max_lag: Maximum lag for mediation testing
    
    Returns:
    - G_pruned: NetworkX DiGraph with mediated edges removed
    - mediation_results: List of mediation analysis results
    """
    edges_to_remove = []
    mediation_results = []
    
    print("Testing for mediated relationships...")
    ...
    for cause, effect in linear_edges:
        ...
        # Remove edge if it's significantly mediated
        if (best_mediator is not None and 
            best_mediation_ratio > mediation_threshold):
            edges_to_remove.append((cause, effect))
            print(f"Removing mediated edge: {cause} → {effect} (mediated by {best_mediator}, ratio: {best_mediation_ratio:.3f})")
    
    # Create pruned graph
    G_pruned = G.copy()
    G_pruned.remove_edges_from(edges_to_remove)
    
    # Add mediation information to remaining edges
    for result in mediation_results:
        cause, effect = result['cause'], result['effect']
        if G_pruned.has_edge(cause, effect):
            G_pruned[cause][effect]['mediation_tested'] = True
            G_pruned[cause][effect]['best_mediation_ratio'] = result['mediation_ratio']
    
    print(f"Removed {len(edges_to_remove)} mediated edges")
    
    return G_pruned, mediation_results
...
def visualize_enhanced_network(G, output_dir, mediation_results=None):
    ...
    # Further separate linear edges by mediation status
    direct_edges = []
    partially_mediated_edges = []
    
    for u, v in linear_edges:
        edge_data = G[u][v]
        mediation_ratio = edge_data.get('best_mediation_ratio', 0)
        
        if mediation_ratio > 0.3:  # Some mediation
    ...
    if direct_edges:
        nx.draw_networkx_edges(G, pos, edgelist=direct_edges, width=3, 
                              ...
                              alpha=0.8, arrows=True, arrowstyle='->', arrowsize=15,
    ...
    if direct_edges:
        legend_elements.extend([
            Line2D([0], [0], color='blue', lw=3, label='Direct Positive Causality'),
            Line2D([0], [0], color='red', lw=3, label='Direct Negative Causality')
        ])
    ...
    if multivar_edges:
        legend_elements.append(
            Line2D([0], [0], color='purple', linestyle='dotted', lw=4, label='Multivariate Causality')
        )
    
    if legend_elements:
        plt.legend(handles=legend_elements, loc='upper right', fontsize=12)
    
    plt.title('Enhanced Causal Network with Mediation Analysis', fontsize=16, fontweight='bold')
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'causal_network_mediation.png'), dpi=300, bbox_inches='tight')
    
    # Display the plot
    plt.show()
    print(f"Enhanced network visualization saved to {os.path.join(output_dir, 'causal_network_mediation.png')}")
...
def create_mediation_summary(mediation_results, output_dir):
    ...
    # Filter for significant mediations
    significant_mediations = mediation_df[mediation_df['mediation_ratio'] > 0.3]
    ...
    if len(significant_mediations) > 0:
        ...
        colors = ['red' if ratio > 0.7 else 'orange' if ratio > 0.5 else 'yellow' 
                 for ratio in mediation_ratios]
        
        plt.barh(y_pos, mediation_ratios, color=colors, alpha=0.7)
        plt.yticks(y_pos, labels)
        plt.xlabel('Mediation Ratio')
        plt.title('Significant Mediation Relationships')
        plt.axvline(x=0.5, color='black', linestyle='--', alpha=0.5, label='50% Mediation')
        plt.axvline(x=0.7, color='red', linestyle='--', alpha=0.5, label='70% Mediation')
        plt.legend()
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'mediation_summary.png'), dpi=300, bbox_inches='tight')
        plt.show()
        
        print(f"Mediation summary saved to {os.path.join(output_dir, 'mediation_summary.png')}")
...
def create_alluvial_diagram(G, output_dir):
    ...
    fig.update_layout(title_text="Causal Flow Diagram (Post-Mediation Analysis)", font_size=10)
    fig.write_html(os.path.join(output_dir, 'causal_flow_mediation.html'))
    fig.show()  # Display the plot
    print(f"Alluvial diagram saved to {os.path.join(output_dir, 'causal_flow_mediation.html')}")
...
def create_causal_matrix(G, output_dir):
    ...
    plt.title('Causal Strength Matrix (Post-Mediation Analysis)')
    plt.xlabel('Effect')
    plt.ylabel('Cause')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'causal_matrix_mediation.png'), dpi=300, bbox_inches='tight')
...
def PhysioNexus(data, exclude_cols=2, corr_threshold=0.6, f_stat_threshold=10, 
    ...
    - mediation_threshold: Minimum mediation ratio to consider edge as mediated (0-1)
    
    Returns:
    - G: NetworkX DiGraph of causal relationships (after mediation analysis)
    ...
    if enable_mediation_analysis and G.number_of_edges() > 2:
        print(f"\nPerforming mediation analysis with threshold {mediation_threshold}...")
        G, mediation_results = prune_mediated_edges(
            G, data_filtered, mediation_threshold=mediation_threshold, max_lag=max_lag)
        print(f"Network after mediation analysis: {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
    else:
    ...
    if G.number_of_nodes() > 0:
        # Enhanced network visualization with mediation info
    ...
    if G.number_of_nodes() > 0 and G.number_of_edges() > 0:
        ...
        if linear_edges:
            ...
            for u, v, data in sorted_linear[:min(5, len(sorted_linear))]:
                mediation_info = ""
                if 'best_mediation_ratio' in data and data['best_mediation_ratio'] > 0.1:
                    mediation_info = f" (mediation: {data['best_mediation_ratio']:.2f})"
                print(f"{u} → {v}: F={data['f_stat']:.2f}, p={data['p_value']:.5f}, correlation={data.get('correlation', 'N/A')}{mediation_info}")
    ...
    if G.number_of_edges() > 0:
        ...
        for u, v, data in G.edges(data=True):
            ...
            if relationship_type == 'linear':
                mediation_ratio = data.get('best_mediation_ratio', 0.0)
                ...
                causal_relationships.append((
                    ...
                    0.0))                      # No mediation for group relationships
        ...
        # Print the DataFrame
        print("\nFinal causal relationships (post-mediation analysis):")
